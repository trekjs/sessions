...
module.exports = class Sessions {
  ...
  matchPath (path, cookiePath) {
    return cookiePath === '/' || path.startsWith(cookiePath)
  }

  async process (ctx, next) {
    if (ctx.sessionId || !this.matchPath(ctx.req.path, this.path)) return next()
    ...
    return this.refresh(ctx.cookies, result)
  }
